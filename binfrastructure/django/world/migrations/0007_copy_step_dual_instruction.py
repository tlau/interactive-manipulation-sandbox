...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Copy the relation in "program" and "instruction_number"."""
        for action in orm['world.BIFAction'].objects.all():
            action.program_as_steps = action.program
            action.step_number = action.instruction_number
            action.save()

    def backwards(self, orm):
        """Copy the relation in "program_as_step" and "step_number"."""
        for action in orm['world.BIFAction'].objects.all():
            action.program = action.program_as_steps
            action.instruction_number = action.step_number
            action.save()
